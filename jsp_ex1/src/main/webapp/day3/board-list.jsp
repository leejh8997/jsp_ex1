...
<script>
	...
	function fnNumber(num){        
		let currentPage = new URLSearchParams(window.location.search).get("page");  // 현재 페이지 번호 가져오기
        if (!currentPage) {
            currentPage = 1;  // 페이지 번호가 없으면 기본값 1
        }
        
        // 페이지 번호와 페이지 크기를 함께 전달
        location.href = "&size=" + num + "?page=" + currentPage;
	}
